...
export class PaymentProcessorRouter {
  ...
  async processPaymentWithRetry(
    ...
    try {
      ...
      // Se falhou no default, tenta o fallback
      if (processor.type === 'default' && retryCount === 0) {
        return this.processPaymentWithRetry(
          payment,
          requestedAt,
          this.processors.fallback,
          1
        )
      }
      
      // Se falhou no fallback ou já tentou ambos, faz retry no mesmo processor
